...
許容誤差 $\varepsilon = 10^{-8}$ を満たすまでの反復過程を示す．

まず二分法について，反復回数 $n=1$ から $n=16$ までの各段階で得られた近似解および誤差の値を表にまとめた．
近似解は初期区間に対応する値から始まり，16 回目の反復において $x_{16}\approx 8.241425\times 10^{-1}$ に到達した．
誤差指標は初期の $2.5\times 10^{-1}$ から反復ごとに約半分ずつ減少し，16 回目には約 $7.6\times 10^{-6}$ となった．

次にニュートン法について，同様に反復ごとの近似解と誤差を集計した．
近似解は 5 回の反復で $x_5\approx 8.241323\times 10^{-1}$ に到達し，この値を解の近似として採用した．
誤差は 1 回目の約 $4.2\times 10^{-1}$ から 5 回目の約 $1.1\times 10^{-10}$ まで反復ごとに大きく減少し，
