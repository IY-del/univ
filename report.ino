...
// 1. シンプルインクリメント (ランナー)
uint8_t gen_inc(uint8_t current) {
  return (current + 1) % STATE_COUNT;
}

// 2. 簡易LFSR的 (擬似ランダム巡回)
uint8_t gen_lfsr(uint8_t current) {
  if (current == 0) current = 1;

  uint8_t x = current & 0x1F; // 5bit
  bool b = ((x >> 0) ^ (x >> 2)) & 1;
  x = (x >> 1) | (uint8_t(b) << 4);

  if (x == 0) x = 1;
  uint8_t next = x % STATE_COUNT;
  return next;
}

// 3. モジュロ乗算的ステップ
uint8_t gen_mod(uint8_t current) {
  const uint8_t a = 5; // N=24に対してそれなりに散る値
  uint8_t next = (uint8_t)((current * a + 1) % STATE_COUNT);
  return next;
}
...
int map_distance_to_duration(int distance_cm) {
  if (distance_cm <= (int)DIST_MIN_CM) return DURATION_MIN_MS;
  if (distance_cm >= (int)DIST_MAX_CM) return DURATION_MAX_MS;
