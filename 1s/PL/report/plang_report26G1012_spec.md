...
任意の高さ・幅にパディングした形で格納する機能を提供するライブラリである．
ファイル中の「0/1」を連続的に読み取り，`h*w`個ごとに1文字分としてまとめることで，
複数文字分のフォントパターン配列を生成する．8×8専用の関数も別途提供する．
...
| 関数名          | 説明                                                       |
| --------------- | ---------------------------------------------------------- |
| load_txt        | テキストファイルから任意の高さ・幅のパターン配列を読み込む |
| load_txt_to_8x8 | `load_txt`を用いて8×8固定のパターン配列を読み込む          |
| free_memory     | `load_txt`系の関数で確保したメモリを解放する               |

## load_txt

テキスト形式のフォントデータファイルを読み込み，
ファイル中の'0'および'1'文字を抽出して`h*w`個ごとに1文字分のパターンとして詰める関数である．
...
| h            | size_t        | 1文字あたりの高さ                           |
| w            | size_t        | 1文字あたりの幅                             |
| out          | int \*\*      | 出力されるパターン配列（1次元）へのポインタ |
...
if (load_txt("font_data01.txt", 8, 8, &patterns, &n) != 0) {
    /* エラー処理 */
}

/* patterns[c*8*8 + r*8 + col] で各セルにアクセス可能 */
...
`font_data01.txt`に含まれる0/1列から，指定した`h*w`サイズごとに複数文字分のパターン配列が生成される．

---

## load_txt_to_8x8

`load_txt`を`h=8, w=8`で呼び出し，戻り値のポインタを`int (*)[8][8]`にキャストする薄いラッパー関数である．
既存の8×8前提のコード（`main.c`など）との互換性を保つために提供する．

| パラメータ名 | 型           | 意味                               |
| ------------ | ------------ | ---------------------------------- |
| filename     | const char[] | フォントデータファイル名           |
| out          | int (\*\*)   | 出力されるパターン配列へのポインタ |
| out_n        | size_t \*    | 読み込んだパターン数（文字数）     |
...
`load_txt`または`load_txt_to_8x8`で確保したメモリを解放する関数である．
引数を`void *`で受けるため，`int *`と`int (*)[8][8]`のどちらの戻り値でもキャスト無しで渡せる．

| パラメータ名 | 型      | 意味                   |
| ------------ | ------- | ---------------------- |
| arr          | void \* | 解放対象のパターン配列 |
...
## print_patternの変更
### new_pattern_view
- デフォルト値をROTATE_0からROTATE_90に変更し，要件の実行例をネイティブにサポート
